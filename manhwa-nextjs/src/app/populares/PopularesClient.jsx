...
/**
 * Mapea una serie raw de la API al formato de ranking de la UI.
 */
...
/**
 * PopularesClient — Componente cliente para la página de Series Populares.
 *
 * Recibe `initialRankings` del Server Component (SSR), lo que garantiza
 * que los crawlers sin JavaScript (OpenAI, Perplexity, etc.) vean el
 * contenido completo desde el primer byte de HTML.
 */
export default function PopularesClient({ initialRankings = [] }) {
    ...
    // Usar datos SSR directamente — sin useEffect, sin loading state
    const rankingsData = initialRankings;
    const loading = false;
    const error = rankingsData.length === 0 ? null : null;
    ...
    const stats = useMemo(() => {
        if (sortedRankings.length === 0) return { totalViews: 0, avgRating: '0.00', totalLikes: 0, totalChapters: 0 };
        return {
    ...
    return (
        ...
        >
            <Container size="lg" py="xl" className="siteContainer">
                ...
                {/* Loading State - solo si no hay datos SSR */}
                {loading && (
                    <Center py="xl">
                                        ...
                                        >
                                            ...
                                            >
                                                <Image
                                                    ...
                                                    alt={`Portada del manhwa popular ${item.title} - Puesto #${item.rank} en el ranking de Manhwa Imperial, calificación ${item.rating} estrellas`}
                                                    style={{
                                                        width: '100%',
                                                        height: '100%',
                                                        objectFit: 'cover',
                                        ...
                                        >
                                            <Grid gutter="lg" align="stretch">
                                                ...
                                                <Grid.Col span={{ base: 12, xs: 2 }}>
                                                    <Image
                                                        src={normalizeImageUrl(item.cover)}
                                                        alt={`Portada del manhwa ${item.title} - Posición #${item.rank} entre los más populares, ${item.chapters} capítulos disponibles en español`}
                                                        style={{
