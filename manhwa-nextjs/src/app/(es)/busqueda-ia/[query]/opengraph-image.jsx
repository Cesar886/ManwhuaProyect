...
// OG image dinámica para /busqueda-ia/[query].
// Consulta el motor IA con la query real, toma las 3 primeras portadas y las compone
// junto al título de búsqueda + branding "IA Imperial".
//
// Next.js usa este archivo automáticamente como openGraph.images para la ruta.
// Los scrapers sociales (Twitter/Facebook/LinkedIn/WhatsApp) cachean agresivamente
// la URL del og:image, así que el coste por-share después del primer fetch es 0.

export const alt = 'Manhwa Imperial — IA Imperial: resultados de búsqueda';
export const contentType = 'image/png';
export const size = { width: 1200, height: 630 };
// Cachear la imagen generada 24h en el CDN; los resultados AI para una misma query
// raramente cambian en ese plazo.
...
export default async function OpengraphImage({ params }) {
    ...
    const displayTitle = humanQuery || 'Recomendaciones IA';
    const covers = await fetchTopResults(humanQuery, 'es');

    // Placeholders para los huecos cuando la IA devuelve menos de 3
        ...
        (
            <div
                style={{
                    ...
                    padding: '56px 72px',
                    background: `radial-gradient(ellipse at top right, ${PALETTE.bgHighlight} 0%, ${PALETTE.bg} 60%), ${PALETTE.bg}`,
                    color: PALETTE.text,
                    fontFamily: 'system-ui, sans-serif',
                    position: 'relative',
                }}
            >
                {/* Header: branding + badge IA */}
                ...
                <div style={{ display: 'flex', gap: '24px', alignItems: 'flex-end' }}>
                    ...
                    >
                        <span style={{ fontSize: '24px', color: PALETTE.accent, fontWeight: 700 }}>
                            Top {covers.length || 3} recomendaciones
                        </span>
                        <span style={{ fontSize: '18px', color: PALETTE.subtext }}>
                            Motor semántico · Catálogo en español
                        </span>
                    </div>
                </div>
            </div>
